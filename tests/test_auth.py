...
from portunus.backend import GcloudBackend
...
def test_gcloud_backend_passes_short_lived_token_file_and_deletes_it(home, monkeypatch):
    ...
    def runner(cmd, capture_output, text, timeout):
        ...
        observed["token_path"] = token_path
        observed["token_text"] = token_path.read_text()
        return SimpleNamespace(returncode=0, stdout="secret-value", stderr="")

    auth = GCPWorkloadIdentityAuth(
        audience="//iam.googleapis.com/projects/1/locations/global/workloadIdentityPools/p/providers/p",
        token_source=StaticOIDC(),
        audit=AuditChain(),
        transport=lambda url, data, headers, timeout: {
            "access_token": "GCP.ACCESS.TOKEN",
            "expires_in": 900,
        },
    )
    backend = GcloudBackend(credential_provider=auth, runner=runner)
    assert backend.access("dostal-x") == "secret-value"
    assert observed["token_text"] == "GCP.ACCESS.TOKEN"
    assert not observed["token_path"].exists()
